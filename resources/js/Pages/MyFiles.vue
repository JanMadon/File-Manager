        ...
        <nav class="flex items-center justify-between p-1 bm-3">
            <ol class="inline-flex items-center space-x-1 md:space-x-1 ">
                <li v-for="ans of ancestors.data" :key="ans.id" class="inline-flex items-center">
                    <Link v-if="!ans.parent_id" :href="route('myFiles')" class="px-2 rounded bg-gray-200 ">
                    My Files
                    </Link>
                    <div v-else class="flex items-center">
                        ...
                        <Link :href="route('myFiles', { folder: ans.path })" class="px-2 rounded bg-gray-200 ">
                        {{ ans.name }}
                        </Link>
                    </div>
            ...
            <div class="flex">
                <label class="flex items-center mr-3">
                    Only Favorites
                    <Checkbox @change="showOnlyFavorites()" v-model:checked="onlyFavorites" class="ml-1"/>
                </label>
                    ...
                    <tr v-for="file of allFiles.data" :key="file.id"
                        ...
                        <td class="px-6 py-4 whitespace-nowrap text-sm font-medium  text-yellow-500" >
                            <div @click.stop.prevent="addRemoveFavorite(file)">
                                <svg v-if="!file.is_favorite" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-6 h-6">
...
function addRemoveFavorite(file) {
    httpPost(route('file.addToFavourites'), {id: file.id})
        .then(() => {
            ...
            showSuccesNotification('Selected files have been added to favourites')
        })
        .catch(async (er) => {
...
function showOnlyFavorites() {
    ...
    params.set('favorites', onlyFavorites.value ? 1 : 0)
